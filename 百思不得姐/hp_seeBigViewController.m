...
@interface hp_seeBigViewController ()<UIScrollViewDelegate>
@property (nonatomic, weak) UIImageView *imageView;
@property (weak, nonatomic) IBOutlet UIButton *saveButton;
@end


@implementation hp_seeBigViewController
...
- (void)viewDidLoad {
    ...
    [image sd_setImageWithURL:[NSURL URLWithString:self.tiezi.large_image] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        if (image == nil) return; // 图片下载失败
        self.saveButton.enabled = YES;
    }];
    [scrollview addSubview:image];
    
    image.xmg_width=scrollview.xmg_width;
    image.xmg_height=self.tiezi.height*image.xmg_width/self.tiezi.width;
    image.xmg_x=0;
    
    if(image.xmg_height >= scrollview.xmg_height) { // 图片高度超过整个屏幕
        image.xmg_y = 0;
        scrollview.contentSize = CGSizeMake(0, image.xmg_height);
    } else { // 居中显示
        image.xmg_centerY = scrollview.xmg_height * 0.5;
    }
    self.imageView = image;
    
    // 缩放比例
    CGFloat scale =  self.tiezi.width / image.xmg_width;
    if (scale > 1.0) {
...
- (IBAction)save {
    ...
    if (status == PHAuthorizationStatusRestricted) { // 因为家长控制, 导致应用无法方法相册(跟用户的选择没有关系)
        ...
        HPLog(@"提醒用户去[设置-隐私-照片-xxx]打开访问开关");
...
- (PHAssetCollection *)createdAssetCollection
{
    ...
    for (PHAssetCollection *assetCollection in assetCollections) {
        if ([assetCollection.localizedTitle isEqualToString:@"百思不得姐"]) {
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] performChangesAndWait:^{
        // 创建相簿的请求
        assetCollectionLocalIdentifier = [PHAssetCollectionChangeRequest creationRequestForAssetCollectionWithTitle:@"百思不得姐"].placeholderForCreatedAssetCollection.localIdentifier;
    } error:&error];
